...
| Idea | Target level | Status | Source | Notes |
|---|---|---|---|---|
| Box2D contact and sensor events | Beginners | Idea | in-repo | The library side is **built** - see [box2d-library.md](plans/box2d-library.md); the wrapper extraction that used to block this is done. Sensor events are already exercised by the gate in [`E06_Box2D_JunkyardInteractive`](../examples/code-only/E06_Box2D_JunkyardInteractive), but that example is a playground, not a lesson. What is still missing is a small beginner-level example that does one thing: a trigger volume and a collision callback, with the per-fixture `enableSensorEvents` requirement made obvious. |
...
| Bepu constraints - servo vs motor vs limit | Beginners | **Done** | BepuSample `Constraint.sdscene`, `Cube Mixer.sdscene` | Built as [`E05_3D_Constraints_Motors`](../examples/code-only/E05_3D_Constraints_Motors). Findings written up in [bepu-constraints.md](../docs/manual/physics-extensions/bepu-constraints.md). |
| A rope from a chain of constraints | Intermediate | **Done** | bepuphysics2 `RopeStabilityDemo.cs` (primary); BepuSample `Ropes.sdscene` | Built as [`E05_3D_Constraints_Rope`](../examples/code-only/E05_3D_Constraints_Rope), following the upstream demo rather than the Stride sample - `DistanceLimit` with zero lever arms, not ball sockets plus swing limits. |
...
| Cube Mixer | - | Declined | BepuSample, `Assets/Shared/Scenes/Cube Mixer.sdscene` | Not a separate example: the Bepu plan consumed this scene as a source for `E05_3D_Constraints_Motors` (the mixer blade is the hinge-plus-motor demonstration). |
...
| Gravity gun - pick up, hold and release a body | Intermediate | Agreed | BepuSample `Components/Utils/GravityGunComponent.cs` | Claimed by the Bepu plan as `E05_3D_Constraints_GravityGun` (Tier 2). Note it is constraint-driven - linear plus angular servo - which is why it joins the constraint family rather than being a raycast example. |
| Trigger volumes | Beginners | Agreed | BepuSample `Components/Utils/TriggerUsageComponent.cs` | Claimed by the Bepu plan as `Example16_TriggerZones` (Tier 2), together with contact events. The plan notes `E13_SignalR` already contains a working `IContactEventHandler`, but nobody looking for collision detection would find it inside a networking example. |
| Overlap queries | Beginners | Agreed | BepuSample `Components/Utils/OverlapTesterComponent.cs` | Claimed by the Bepu plan as `Example14_ShapeQueries` (Tier 2), family-grouped with `E05_3D_Raycast`. |
| Shape sweep queries | Intermediate | Agreed | bepuphysics2, `SweepDemo.cs` | Same example as overlap above - `Example14_ShapeQueries`. Confirmed exposed: `BepuSimulation.SweepCast` and `SweepCastPenetrating`, both taking a `CollisionMask`. The plan also adds `RayCastPenetrating` to the existing `E05_3D_Raycast` alongside it. |
...
| Character controller | Intermediate | **Done** | in-repo | Already covered by [`E05_3D_FirstPersonCharacter`](../examples/code-only/E05_3D_FirstPersonCharacter), which drives Stride's `CharacterComponent` from a `FirstPersonControllerComponent` and its processor. The third-person camera row under Input is the remaining gap, not the controller itself. |
