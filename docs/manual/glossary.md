...
- **Code-only**: A Stride game written entirely in C#, with no Game Studio and no scene or asset files; the toolkit's extensions exist to make that short. See [Code-Only](code-only/index.md).
- **Example**: One runnable project under `examples/code-only`, teaching one idea, with a metadata block at the bottom of `Program.cs` that the docs and launchers read. See the [examples gallery](code-only/examples/index.md) and [contributing examples](../contributing/examples/index.md).
...
- **Manual**: These pages. The [Stride manual](https://doc.stride3d.net/latest/en/manual/index.html) covers the engine; this one covers what the toolkit adds and the engine behaviour it ran into.
...
- **Stride Community Toolkit**: A set of libraries, examples and documentation maintained by the community, filling in what the engine leaves to you when working from code. See [Getting started](getting-started.md).
...
- **Delta time**: The seconds since the last frame, read from `GameTime`. Multiply movement by it so speed does not depend on frame rate. See [Delta time](https://doc.stride3d.net/latest/en/tutorials/csharpbeginner/delta-time.html).
...
- **Entity-component system (ECS)**: The pattern Stride is built on: entities hold components, and processors do the work for every component of a kind. See [Components and scripts](components-and-scripts.md).
...
- **GameTime**: The clock object passed to every update: total time, delta time, frame count. See the [API](https://doc.stride3d.net/latest/en/api/Stride.Games.GameTime.html).
...
- **Processor**: A system that runs once per frame over every component of one type. The right home for behaviour shared by many entities, and what makes a component work in Game Studio. See [Components and scripts](components-and-scripts.md) and [Components in Game Studio](../contributing/toolkit/game-studio-components.md).
...
- **Primitive**: A built-in shape such as a cube, sphere, capsule, cone or torus, generated from a few numbers. `game.Create3DPrimitive(...)` builds one with a material and a collider. See [Engine patterns](engine-patterns.md).
...
- **Runtime font**: A font rasterised on the fly from a system or embedded TTF, with no font asset; how every text feature in a code-only game gets its glyphs. See [Engine patterns](engine-patterns.md).
...
- **Sprite font**: A texture of pre-drawn glyphs the engine draws text from. See [Sprite fonts](https://doc.stride3d.net/latest/en/manual/graphics/sprite-fonts.html).
...
- **HDR**: Rendering with colour values above 1, so a bright light or an emissive material can be brighter than white, then tone-mapped down for the screen. Stride's default pipeline is HDR. See [Post effects](https://doc.stride3d.net/latest/en/manual/graphics/post-effects/index.html).
- **HUD**: Text and shapes drawn on the screen rather than in the world: score, health, crosshair. Screen-space ShapeBatch and the debug overlay draw one. See [ShapeBatch](rendering/shape-batch.md).
...
- **MeshBuilder**: The toolkit's fluent builder for meshes from code: choose a layout, add vertices and indices, get a mesh. See [MeshBuilder](rendering/mesh-builder.md).
...
- **Render view**: One camera's worth of a frame in the render system: what it sees, sorted and culled. A scene renderer owns one. See [Custom scene renderers](https://doc.stride3d.net/latest/en/manual/graphics/graphics-compositor/custom-scene-renderers.html).
...
- **Wireframe**: Drawing only triangle edges. Stride has a rasterizer state for it; the toolkit's mesh examples show it. See [Mesh outline](code-only/examples/mesh-outline.md).
...
- **Effects package**: `Stride.CommunityToolkit.Effects`, where the toolkit keeps everything with a shader in it: post-processing colour transforms and GPU picking. See [GPU picking](rendering/gpu-picking.md).
...
- **Shader compilation**: Turning SDSL into what the graphics driver runs, which Stride does at runtime and caches; the first frame that needs a new shader pays for it, which is why captures wait a few frames. See [Compile shaders](https://doc.stride3d.net/latest/en/manual/graphics/effects-and-shaders/compile-shaders.html).
...
- **Simulation space**: Whether particles keep their world position after birth (world space) or move with the emitter (local space). See [Emitters](https://doc.stride3d.net/latest/en/manual/particles/emitters.html).
...
- **Convex hull**: The tightest convex shape around a mesh, used as a collider for shapes that are not primitives; one hull per body is expensive, so the toolkit shares them. See [Who owns the transform?](physics-extensions/bepu-transform-ownership.md).
...
- **NaN**: "Not a number", the value a float becomes after dividing by zero or overflowing; once a body's position is NaN it is lost, and physics engines fail loudly around it. See [Who owns the transform?](physics-extensions/bepu-transform-ownership.md).
...
- **AABB** (bounding box): The smallest axis-aligned box around an object; cheap to test, so engines use it before anything precise. See [BoundingBox](https://doc.stride3d.net/latest/en/api/Stride.Core.Mathematics.BoundingBox.html).
...
- **Interpolation** (lerp): Blending from one value to another by a fraction between 0 and 1. `MathUtilEx.Interpolate` does it with an easing. See [MathUtil](https://doc.stride3d.net/latest/en/api/Stride.Core.Mathematics.MathUtil.html).
...
- **Radians**: The angle unit every engine function takes; a full turn is 2π, and `MathUtil.DegreesToRadians` converts. See [MathUtil](https://doc.stride3d.net/latest/en/api/Stride.Core.Mathematics.MathUtil.html).
- **Tween**: A small clock over one easing curve: start it, feed it the frame time, read a value between two of your own. The toolkit's `Tween` is that and nothing more; see [Easing in a 3D Game](code-only/examples/easing-3d-game.md).
...
- **Immediate mode**: Drawing or building UI by calling functions every frame, with no retained objects; ImGui, debug shapes and ShapeBatch all work this way.
- **Myra**: A retained-mode UI library for Stride, with windows, buttons and layouts. See the [Myra example](code-only/examples/myra-ui-draggable-window-and-services.md).
...
- **Screenshot capture**: An environment variable that makes any example save a frame and exit, which is how the docs gallery pictures and the golden images are made. See [Contribute examples](../contributing/examples/index.md).
- **WARP**: Windows' software Direct3D renderer, the same on every machine, so golden images captured with it match on any PC and on CI. See [Shaders in a package](../contributing/toolkit/shaders.md).